"""insight.roadshow_list — 路演日程列表（分页, 端点最大页 50, 返回 DataFrame）。

通过多组示例覆盖全部参数；可选参数的枚举值已在注释中标注（取自 gangtise CLI 文档, 未杜撰）。
异步用法相同, 路径为 gangtise.async_.insight.roadshow_list(...)。
"""
...
async def main():
    # 示例 1 · 最简调用: 取最近 5 条
    show_result(await gangtise.async_.insight.roadshow_list(size=5), __file__)

    # 示例 2 · 常用过滤: 时间窗 + 关键词 + 证券 + 对象类型
    show_result(
        await gangtise.async_.insight.roadshow_list(
    ...
    #   category=<分类>               分类名称过滤, 支持单值或列表; 取值 earningsCall/strategyMeeting 等
