...
async def main():
    # 示例 1 · 最简调用: 取最近 5 条
    show_result(await gangtise.async_.insight.opinion_list(size=5), __file__)

    # 示例 2 · 常用过滤: 关键词 + 时间窗 + 行业, 指定排序
    show_result(
        await gangtise.async_.insight.opinion_list(
    ...
    #   concept=<题材ID>             见 reference.concept_search(keyword=...)（如 机器人=121000130）
