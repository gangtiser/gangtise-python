"""fundamental.income_statement_us — 美股利润表，返回 DataFrame。

通过多组示例覆盖全部参数；可选参数的枚举值已在注释中标注（取自 gangtise CLI 文档, 未杜撰）。
异步路径为 gangtise.async_.fundamental.income_statement_us(...)（同步用法见 sample/sync 同名文件）。
"""
...
async def main():
    # 示例 1 · 最简调用: 仅传必填美股代码
    show_result(
        await gangtise.async_.fundamental.income_statement_us(security_code="AAPL.O"),
        __file__,
    )

    # 示例 2 · 时间窗 + 报告期过滤
    show_result(
        await gangtise.async_.fundamental.income_statement_us(
