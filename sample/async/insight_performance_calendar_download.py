...
异步路径为 gangtise.async_.insight.performance_calendar_download(...)（同步用法见 sample/sync 同名文件）。
"""
...
async def main():
    ...
    items = await gangtise.async_.insight.performance_calendar_list(
    ...
    try:
        os.chdir(output_dir)
        result = await gangtise.async_.insight.performance_calendar_download(
