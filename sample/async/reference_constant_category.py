...
异步路径为 gangtise.async_.reference.constant_category(...)（同步用法见 sample/sync 同名文件）。
"""
...
async def main():
    ...
    show_result(await gangtise.async_.reference.constant_category(), __file__)

    # 示例 2 · 原始返回: usageScopes（apiName + paramName 嵌套结构）在 raw 下更易读
    show_result(
        await gangtise.async_.reference.constant_category(
