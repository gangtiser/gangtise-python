...
async def main():
    # 示例 · 取「机器人」主题的最新画像
    show_result(
        await gangtise.async_.alternative.concept_info(
    ...
    #     await gangtise.async_.alternative.concept_info(concept_id="121000342")
    #   raw=True 仅为签名统一而保留, 返回内容与默认一致(同为该 dict):
    #     await gangtise.async_.alternative.concept_info(concept_id="121000130", raw=True)
