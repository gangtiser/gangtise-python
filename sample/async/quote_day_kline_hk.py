"""quote.day_kline_hk — 港股日 K 线（.HK）, 返回 DataFrame。

通过多组示例覆盖全部参数；可选参数的取值范围已在注释中标注（取自 gangtise CLI 文档, 未杜撰）。
security="all" 时按日期窗口自动分片（每片 2 个交易日）并并发拉取后合并。
异步路径为 gangtise.async_.quote.day_kline_hk(...)。
"""
...
async def main():
    # 示例 1 · 最简调用: 取单只港股最近一段日 K 线
    show_result(
        await gangtise.async_.quote.day_kline_hk(
            security="00700.HK",  # 港股代码, 后缀 .HK; 支持单值或列表; "all"=全市场
    ...
    show_result(
        await gangtise.async_.quote.day_kline_hk(
            security=["00700.HK", "01913.HK"],  # 支持单值或列表
    ...
    # 其余可选用法:
    #   security="all"            全市场: 配合 start_date/end_date 自动按 2 交易日/片分片并发拉取
