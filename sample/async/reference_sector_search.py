...
异步路径为 gangtise.async_.reference.sector_search(...)（同步用法见 sample/sync 同名文件）。
"""
...
async def main():
    ...
    show_result(
        await gangtise.async_.reference.sector_search(
    ...
    # 示例 2 · 不传关键词: 返回默认板块列表
    show_result(await gangtise.async_.reference.sector_search(), __file__)
