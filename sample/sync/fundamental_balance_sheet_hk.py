"""fundamental.balance_sheet_hk — 港股资产负债表（中国准则口径），返回 DataFrame。

通过多组示例覆盖全部参数；可选参数的枚举值已在注释中标注（取自 gangtise CLI 文档, 未杜撰）。
异步用法相同, 路径为 gangtise.async_.fundamental.balance_sheet_hk(...)。
"""
...
def main():
    # 示例 1 · 最简调用: 仅传必填港股代码
    show_result(
        gangtise.fundamental.balance_sheet_hk(security_code="00700.HK"),  # 单个港股代码
        __file__,
    )

    # 示例 2 · 时间窗 + 报告期过滤
    show_result(
        gangtise.fundamental.balance_sheet_hk(
