"""fundamental.cash_flow — A股现金流量表（累计口径），返回 DataFrame。

通过多组示例覆盖全部参数；可选参数的枚举值已在注释中标注（取自 gangtise CLI 文档, 未杜撰）。
异步路径为 gangtise.async_.fundamental.cash_flow(...)。
"""
...
async def main():
    # 示例 1 · 最简调用: 仅传必填证券代码
    show_result(
        await gangtise.async_.fundamental.cash_flow(security_code="000001.SZ"),  # 单个证券代码
        __file__,
    )

    # 示例 2 · 时间窗 + 财年过滤
    show_result(
        await gangtise.async_.fundamental.cash_flow(
