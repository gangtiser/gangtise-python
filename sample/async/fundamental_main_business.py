...
异步路径为 gangtise.async_.fundamental.main_business(...)。
"""
...
async def main():
    ...
    show_result(
        await gangtise.async_.fundamental.main_business(security_code="600519.SH"),  # 单个证券代码
        __file__,
    )

    # 示例 2 · 按行业拆分 + 时间窗 + 报告期
    show_result(
        await gangtise.async_.fundamental.main_business(
