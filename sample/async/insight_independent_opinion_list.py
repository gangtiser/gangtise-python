...
async def main():
    # 示例 1 · 最简调用: 取最近 5 条
    show_result(await gangtise.async_.insight.independent_opinion_list(size=5), __file__)

    # 示例 2 · 常用过滤: 时间窗 + 关键词 + 美股 + 行业, 时间倒序
    show_result(
        await gangtise.async_.insight.independent_opinion_list(
            ...
            industry=104110000,  # 申万行业 ID（104xx0000 形态）, 支持单值或列表
        ),
        __file__,
    )
