...
异步用法相同, 路径为 gangtise.async_.insight.pamirs_summary_download(...)。
"""
...
def main():
    ...
    items = gangtise.insight.pamirs_summary_list(size=1)
    if items.empty:
    ...
    try:
        os.chdir(output_dir)
        result = gangtise.insight.pamirs_summary_download(
