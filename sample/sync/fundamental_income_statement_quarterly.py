"""fundamental.income_statement_quarterly — A股利润表（单季口径），返回 DataFrame。

通过多组示例覆盖全部参数；可选参数的枚举值已在注释中标注（取自 gangtise CLI 文档, 未杜撰）。
异步用法相同, 路径为 gangtise.async_.fundamental.income_statement_quarterly(...)。
"""
...
def main():
    # 示例 1 · 最简调用: 仅传必填证券代码
    show_result(
        gangtise.fundamental.income_statement_quarterly(security_code="000001.SZ"),  # 单个证券代码
        __file__,
    )

    # 示例 2 · 时间窗 + 单季报告期过滤
    show_result(
        gangtise.fundamental.income_statement_quarterly(
