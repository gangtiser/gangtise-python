...
异步路径为 gangtise.async_.insight.report_image_download(...)（同步用法见 sample/sync 同名文件）。
"""
...
async def main():
    ...
    items = await gangtise.async_.insight.report_image_list(keyword="AI", top=1)
    if items.empty:
    ...
    try:
        os.chdir(output_dir)
        result = await gangtise.async_.insight.report_image_download(
